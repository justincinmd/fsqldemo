...
public class Producer implements Runnable, AutoCloseable {
  ...
  public void run() {
    ...
    KafkaProducer<Long, Account> accountProducer = new KafkaProducer<>(getAccountProperties());

    Throttler throttler = new Throttler(100);

    TransactionSupplier transactions = new TransactionSupplier();
    AccountSupplier accounts = new AccountSupplier();

    for (int i=0; i<5; i++) {
      Account account = accounts.get();
      long millis = account.timestamp.atZone(ZoneOffset.UTC).toInstant().toEpochMilli();
      ProducerRecord<Long, Account> accountRecord =
              new ProducerRecord<>("accounts", null, millis, account.accountId, account);
      accountProducer.send(accountRecord);
    }

    long count = 0;
    Account updatingAccount = accounts.get();
    while (isRunning) {
      count++;
      ...
      if (count % 100 == 0) {
        updatingAccount.accountName = "Account Iter " + count;
        ProducerRecord<Long, Account> accountRecord =
                new ProducerRecord<>("accounts", null, millis, updatingAccount.accountId, updatingAccount);
        accountProducer.send(accountRecord);
      }
